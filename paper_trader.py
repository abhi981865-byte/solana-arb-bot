...
# --- Learning memory ---
MAX_LEARNINGS = 50  # keep the most recent N learning notes
...
def record_learning(state, text):
    """
    Appends a timestamped learning note (auto-generated or manually added via
    Telegram) so the bot can be reminded of past mistakes/patterns in future
    chat context. Keeps only the most recent MAX_LEARNINGS notes. Saves
    immediately so the note isn't lost even if the run crashes right after.
    """
    state.setdefault("learnings", [])
